...
import type { Unite, StatutUnite } from '@/types'
...
export default function VentesClient({ unites, role }: Props) {
  ...
  const zones = Array.from(new Set(unites.map(u => u.zone))).sort()
  const types = Array.from(new Set(unites.map(u => u.type)))
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {(['vendu','reserve','disponible'] as StatutUnite[]).map(s => (
          <button key={s} onClick={() => setStatutF(statutF === s ? 'tous' : s)}
      ...
      <div className="card p-4 flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-40">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher une référence…" className="input pl-8" />
        </div>
      ...
      <div className="flex gap-6">
        <div className="card p-6 flex-1">
          <div className="grid grid-cols-[repeat(auto-fill,minmax(52px,1fr))] gap-2">
            {filtered.map(u => (
              <button key={u.id} onClick={() => setSelected(selected?.id === u.id ? null : u)}
                title={u.reference}
                className={cn('aspect-square rounded-lg text-xs font-bold flex flex-col items-center justify-center gap-0.5 border-2 transition-all duration-150 hover:scale-110',
                  selected?.id === u.id ? 'border-rc-600 scale-110 shadow-lg' : 'border-transparent')}
                style={{ background: COULEURS_STATUT_UNITE[u.statut] }}>
                <span style={{ fontSize: '10px' }}>{u.reference.replace('RC-','')}</span>
              </button>
            ))}
            {filtered.length === 0 && (
              <div className="col-span-full text-center py-10 text-rc-400 text-sm">Aucune unité ne correspond aux filtres</div>
            )}
          </div>
        ...
        {selected && (
          <div className="card p-6 w-72 shrink-0 self-start sticky top-6">
            <div className="flex items-start justify-between mb-4">
              ...
              <span className={cn('badge', statutUniteCouleur(selected.statut))}>{LABELS_STATUT_UNITE[selected.statut]}</span>
            </div>
            ...
            {canEdit && selected.statut === 'disponible' && (
              <a href={`/contrats/nouveau?unite=${selected.id}`} className="btn-primary block text-center mt-4 text-sm">Créer une réservation</a>
            )}
          </div>
        )}
      </div>
